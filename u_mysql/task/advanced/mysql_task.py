...
# 학부모 회원가입___________________________________________________________________________
...
# gender = input('성별: ')
#
# # 위 5개 항목이 아래 parent_datas 안에 [] 리스트로 들어온다.
#
# parent_datas = [name, age, address, phone, gender]
# # 여기서는 param 이 parent_datas이 된다.
# # 2시간 20분 부터 빡집중 해서 듣기
# # parant_datas에 전부 담는다.
#
...
# # 아이가 있다면 아이의 정보까지 입력
# #  방금 입력 한 학부모의 번호가 필요 하다

# 아랫 줄 살리지 마세요
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#   find_current_sequence_query = "select last_insert_id();"
#   parent_id = find_current_sequence(find-current_sequence_query)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

insert_query = "insert into tbl_child(child_name, child_age, child_gender, parent_id) " \
               "values(%s, %s, %s, %s)"
# # 아이가 한 명이 아니라 여러 명이 들어올 것이기 때문에 while문 사용
# while True:
#     choice = input("아이 정보를 입력하시겠습니까?[y/N]\n")
#     if choice.upper() == 'N':
#         # .upper() 를 쓰면 소문자로 써도 무조건 대문자로 바꿔준다.
#         break
...
# 인가(Authorization): 권한 부여 => admin 쪽 설명 필요
...
# 당첨 번호 등록 (30개의 당첨 번호 만들기)
# get_count_query = "select count(id) product_count from tbl_product"
#
...
#
# for i in range(30):
#     index = randint(0, get_count(get_count_query).get('product_count') - 1)
