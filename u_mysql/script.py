...
# 강의 4시간 15분 50초 부터 암호화 강의 시작
# 암호화_____________________________________________________
encrypt = hashlib.sha256()
# sha256 => 많이 쓰이는 암호화 방식
encrypt.update('5555'.encode('utf-8'))
# print(encrypt.hexdigest())
#  16진수로 암호화
# DB쪽에서 암호화 하는 것 보다 코드쪽에서 암호화 하는 것이 좋다.

# insert_param = ['lss5555', encrypt.hexdigest(), '이순신']
# ___________________________________________________________암호화

# delete_query = "delete from tbl_member where member_name = %s"
# delete_param = ['이순신']
# 파라매터는 반드시 LIST로 [] 넣어준다.


# find_all_query = "select id, member_id, member_password, member_name from tbl_member"
# 업데이트 ____________________________________________________________
# update_query = "update tbl_member set member_name = %s where member_id = %s"
# update_param = ['한동숙', 'lss5555']
# ________________________________________________________________________

# -회원 번호로 한 명 조회 쿼리!________________________________________________________________
...
# 단일 테이블 3개 제작 후 CRUD 진행__________________________________
# --------------------------------------------------------------
# 1. 상품 정보 추가______________________________________________________________________
